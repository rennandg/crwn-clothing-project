...
export const selectCollectionsForPreview = createSelector(
    [selectCollections],
    collections => Object.keys(collections).map(key => collections[key]) // this show that we want to get all the key of collections and map all the array of keys so we get the value of our collection object
)
...
export const selectCollection = memoize((collectionUrlParam) => 
    createSelector(                 //map over the collection by selecting it then passing into our new selectCollection to the string collectionUrlParam
        ...
        collections => collections[collectionUrlParam]                                  //Base on the function we pass it. it's going to run eact element from left to right in our array
